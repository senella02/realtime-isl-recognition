...
def load_dataset(file_location: str, dataframe: pd.DataFrame = None):
    ...
    def safe_eval(val):
        ...
        return val # ถ้าเป็น list หรือ np.array อยู่แล้ว ให้ส่งคืนเลย
    ...
    current_row = np.empty(shape=(len(safe_eval(df["leftEar_X"])), len(BODY_IDENTIFIERS + HAND_IDENTIFIERS), 2))
    for index, identifier in enumerate(BODY_IDENTIFIERS + HAND_IDENTIFIERS):
        current_row[:, index, 0] = safe_eval(df[identifier + "_X"])
        current_row[:, index, 1] = safe_eval(df[identifier + "_Y"])
...
class CzechSLRDataset(torch_data.Dataset):
    ...
    def __init__(self, dataset_filename: str, dataframe: pd.DataFrame, normalize=True):
        """
        ...
        loaded_data = load_dataset(dataset_filename, dataframe)
        dataset = loaded_data

...
def normalize_body_full(df: pd.DataFrame) -> (pd.DataFrame, list):
    ...
    for index, row in df.iterrows():
        ...
        sequence_size = len(row["leftEar_Y"]) # sequence = เฟรมต่างๆของวิดีโอเรา
        # ทำไมต้องเอา len ของ หูซ้ายไม่รู้แต่เข้าใจว่าต้องการคำนวณว่าวิดีโอนี้มีกี่เฟรม
        valid_sequence = True
        original_row = row
        ...
        for sequence_index in range(sequence_size):
            ...
            # Prevent from even starting the analysis if some necessary elements are not present ค่อยๆไล่เช็คทีละเฟรมไปเลย
            # ถ้าไม่มีไหล่ซ้ายหรือไม่มีไหล่ขวา และ ไม่มีคอหรือไม่มีจมูก จะใช้ไม่ได้ -> แปลง่ายๆคือ อย่างน้อยต้องมีไหล่ซ้ายไหล่ขวาพร้อมกัน หรือถ้าไม่มี ก็ต้องมีจมูกกับไหล่พร้อมกัน
            if (row["leftShoulder_X"][sequence_index] == 0 or row["rightShoulder_X"][sequence_index] == 0) and (row["neck_X"][sequence_index] == 0 or row["nose_X"][sequence_index] == 0):
                ...
                # กรณีมีทั้งไหล่ซ้าย ไหล่ขวา จะประมาณขนาดหัวจากระยะระหว่างไหล่
                ...
                # ถ้าไม่มีไหล่ทั้งสองข้าง จะประมาณขนาดหัวจากระยะคอถึงจมูก
            ...
            for identifier in BODY_IDENTIFIERS:
                ...
                # ขยับจุดให้พิกัดสัมพัทธกับกรอบใหม่
        ...
        if valid_sequence:
            ...
            normalized_df = pd.concat([normalized_df, original_row.to_frame().T], ignore_index=True)
